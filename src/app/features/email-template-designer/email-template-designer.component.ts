...
export class EmailTemplateDesignerComponent implements OnInit {
  ...
  steps: { label: string; value: WizardStep }[] = [{ label: 'Select template', value: 'choose' }, { label: 'Design', value: 'design' }, { label: 'Preview', value: 'preview' }];
  currentStep: WizardStep = 'choose'; selectedStarterId = ''; selection: EmailSelection = null; previewOpen=false; previewWidth:'desktop'|'mobile'='desktop'; generatedHtml=''; savedJson='';
  ...
  clear(){ this.template.sections=[]; this.selection=null; this.selectedStarterId=''; this.regenerate(); }
  ...
  openPreviewStep(){ this.generatedHtml = this.builder.buildHtml(this.template, v => this.previewData.renderTokens(v)); this.currentStep = 'preview'; }
  goTo(step: WizardStep){ if(step === 'preview') this.generatedHtml = this.builder.buildHtml(this.template, v => this.previewData.renderTokens(v)); this.currentStep = step; }
